...
export function browserTabDropAdapter(session: number, onBrowserTabDrop?: CollectionRowsProps["onBrowserTabDrop"]): NonNullable<CollectionListProps["externalDrop"]> {
  return {
    session,
...
export function CollectionRows({ browserTabDragSession = 0, onBrowserTabDrop, onOpenCollection, ...props }: CollectionRowsProps) {
  return <CollectionList {...props}
    ...
    externalDrop={browserTabDropAdapter(browserTabDragSession, onBrowserTabDrop)}
  />;
}
