...
export function CollectionRows({ collections, links, repository, onReload, openLink = (url) => chrome.tabs.create({ url }) }: CollectionRowsProps) {
  ...
  return <div className="ext-columns">
    {orderedCollections.map((collection) => {
      ...
      return <article
        ...
        onDrop={() => dragged?.kind === "collection" ? void moveCollection(collection.id) : void moveLink(collection.id)}
        role="group"
      >
        ...
        <div className="ext-link-grid">
          {collectionLinks.map((link) => <a
            ...
            onDragOver={allowDrop}
            onDrop={(event) => { event.preventDefault(); event.stopPropagation(); void moveLink(collection.id, link.id); }}
