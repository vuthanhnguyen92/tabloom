...
it("does not reopen the destination when the same user's token refreshes", async () => {
  ...
  await userEvent.type(screen.getByRole("searchbox", { name: "Search all spaces and collections" }), "figma");
  await act(async () => { onAuth("TOKEN_REFRESHED", { ...session }); });
  expect(screen.getByRole("searchbox", { name: "Search all spaces and collections" })).toHaveValue("figma");
  expect(screen.getByRole("button", { name: "Open Product launch" })).toHaveAttribute("aria-current", "page");
});
