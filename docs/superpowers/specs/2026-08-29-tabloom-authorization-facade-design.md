...
The live Supabase OAuth 2.1 flow completed successfully with public dynamic client registration and PKCE, but the issued ES256 access token contained only `aud: ["authenticated"]`. It did not contain the requested Tabloom MCP resource, `https://tabloom-mcp.vercel.app`.
...
The Vercel origin `https://tabloom-mcp.vercel.app` serves two isolated subsystems.
...
The authorization server issuer and protected resource identifier are both the exact origin `https://tabloom-mcp.vercel.app`. Endpoint paths do not become token audiences.
